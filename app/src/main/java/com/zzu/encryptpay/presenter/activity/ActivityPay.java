...
import com.zzu.encryptpay.SMS4;
import com.zzu.encryptpay.utils.SignUtils;
...
public class ActivityPay extends FragmentActivity {
            ...
            switch (msg.what) {
                case SDK_PAY_FLAG: {
                    ...
                    if (TextUtils.equals(resultStatus, "9000")) {
                        Toast.makeText(ActivityPay.this, "支付成功", Toast.LENGTH_SHORT).show();
                    } else {
                        // 判断resultStatus 为非"9000"则代表可能支付失败
                        // "8000"代表支付结果因为支付渠道原因或者系统原因还在等待支付结果确认，最终交易是否成功以服务端异步通知为准（小概率状态）
                        if (TextUtils.equals(resultStatus, "8000")) {
                            Toast.makeText(ActivityPay.this, "支付结果确认中", Toast.LENGTH_SHORT).show();

                        } else {
                            // 其他值就可以判断为支付失败，包括用户主动取消支付，或者系统返回的错误
                            Toast.makeText(ActivityPay.this, "支付失败", Toast.LENGTH_SHORT).show();
     ...
     * 加解密结果分析例子
     */
    public void encryptResult(){
        //加密 128bit
        sm4.sms4(in, inLen, key, out, ENCRYPT);
        for(int i1=0;i1<16;i1++){
            Log.i("加密结果：", "" + Integer.toHexString(out[i1]&0xff)+"\t");
        }
        //解密 128bit
        sm4.sms4(out, inLen, key, in, DECRYPT);
        for(int i1=0;i1<16;i1++) {
            Log.i("解密结果：", "" + Integer.toHexString(in[i1] & 0xff) + "\t");
        }

        //加密多个分组
        sm4.sms4(in1, inlen1, key, out1, ENCRYPT);
        for(int i1=0;i1<32;i1++){
            Log.i("加密多个分组：", "" + Integer.toHexString(out1[i1] & 0xff) + "\t");
        }
        //解密多个分组
        sm4.sms4(out1, inlen1, key, in1, DECRYPT);
        for(int i1=0;i1<32;i1++){
            Log.i("解密结果：", "" + Integer.toHexString(out1[i1]&0xff)+"\t");
        }
        //1,000,000次加密
        ...
        sm4.sms4(in, inLen, key, out, ENCRYPT);
        for(int i1=0;i1<16;i1++){
            Log.i("解密结果：", "" + Integer.toHexString(out[i1]&0xff)+"\t");
        }
    ...
    public void h5Pay(View v) {
        Intent intent = new Intent(this, H5PayAppActivity.class);
